...
use crate::domain::chats::{Chat, ChatRole, ChatType, Message, MessageType};
...
pub async fn get_chats(
    ...
    match Chat::find_by_user(user_id, db.get_pool()).await {
        ...
        Err(e) => {
            ...
            Ok(
                HttpResponse::InternalServerError().json(ApiResponse::<Vec<ChatResponse>> {
                    success: false,
...
pub async fn get_chat(db: web::Data<Database>, path: web::Path<i64>) -> Result<HttpResponse> {
    ...
    match Chat::find_by_id(chat_id, db.get_pool()).await {
        ...
        Ok(None) => Ok(HttpResponse::NotFound().json(ApiResponse::<ChatResponse> {
            success: false,
        ...
        Err(e) => {
            ...
            Ok(
                HttpResponse::InternalServerError().json(ApiResponse::<ChatResponse> {
                    success: false,
        ...
        Ok(chat_id) => {
            ...
            for &participant_id in &req.participants {
                if participant_id != creator_id {
                    let _ = chat
                        .add_participant(participant_id, ChatRole::Member, db.get_pool())
                        .await;
                }
            }
...
pub async fn get_messages(
    ...
    let chat_id = path.into_inner();
    let page = query.get("page").and_then(|v| v.as_i64());
    let per_page = query.get("per_page").and_then(|v| v.as_i64());

    debug!(
        "Получение сообщений чата {} (страница: {:?}, на странице: {:?})",
    ...
    match Message::find_by_chat(chat_id, page, per_page, db.get_pool()).await {
        ...
        Err(e) => {
            ...
            Ok(
                HttpResponse::InternalServerError().json(ApiResponse::<Vec<MessageResponse>> {
                    success: false,
...
pub async fn send_message(
    ...
    match Message::new(chat_id, sender_id, req.content.clone(), message_type) {
        Ok(message) => match message.create(db.get_pool()).await {
            Ok(message_id) => Ok(HttpResponse::Created().json(ApiResponse {
                success: true,
        ...
        Err(validation_error) => Ok(HttpResponse::BadRequest().json(ApiResponse::<
            serde_json::Value,
        > {
            success: false,
